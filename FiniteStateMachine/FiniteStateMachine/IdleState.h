...
class IdleState : public State //inherits from state so that it can use all of the precreated variables and functions;
{
public:
	IdleState(DataStore* datastore); //called when the idle state is created and takes a datastore pointer so that it can use data from the data store and pass the datastore to the next state.
	~IdleState();
...
private:
	char inputChar; //Will be assigned a charachter when the user is asked to input.
};
