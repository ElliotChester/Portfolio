...
{
public:
	PrintState(DataStore* datastore); //called when the idle state is created and takes a datastore pointer so that it can use data from the data store and pass the datastore to the next state.
	~PrintState();
